...
function start(mode, type, selection) {
	...
	if (qm.getJob() == 2000) {
		...
		qm.gainSp(1);
	    qm.sendNextS("#b(Tôi c¡m th¬y nhï có g¾ «ó hi½n lên trong «®u...)#k", 3);
	}
    } else if (status == 2) {
	qm.sendYesNoS("B¢n s³ bÇ qua video clip? Ngay c¡ khi b¢n bÇ qua c¡nh này, trÆ chÛi s³ không bÅ ¡nh hïäng.", 1);
    } else if (status == 3) {
	qm.warp(140000000, 0)
	qm.dispose();
    }
