...
//        System.out.println("Replaced pw: " + pwd);
        ...
        if (AutoRegister.autoRegister && !AutoRegister.getAccountExists(login) && (!c.hasBannedIP() || !c.hasBannedMac())) {
            if (pwd.equalsIgnoreCase("disconnect") || pwd.equalsIgnoreCase("fixme")) {
                c.getSession().write(CWvsContext.broadcastMsg(1, "M±t kh¯u không «úng."));
            ...
            if (AutoRegister.success) {
                c.getSession().write(CWvsContext.broadcastMsg(1, "Ð£ng kü tû «Øng hoàn t¬t!\r\nH»y «£ng nh±p «º vào trÆ chÛi."));
                c.getSession().write(LoginPacket.getLoginFailed(1)); //Shows no message, used for unstuck the login button
                return;
            }
            ...
            c.getSession().write(CWvsContext.broadcastMsg(1, "Nh±n v±t cça b¢n «» ng¤t kªt nÑi thành công."));
            c.getSession().write(LoginPacket.getLoginFailed(1)); //Shows no message, used for unstuck the login button
            return;
        } else {
        ...
        if (!c.isGm() && !c.isLocalhost() && ServerConstants.Use_Localhost) {
            c.getSession().write(CWvsContext.broadcastMsg(1, "Chúng tôi r¬t tiªc, nhïng máy chç «ang «ïæc b¡o tr¾, vui lÆng kiºm tra các di¼n «àn «º biªt thêm thông tin."));
            c.getSession().write(LoginPacket.getLoginFailed(1)); //Shows no message, used for unstuck the login button
        }
        ...
        if (loginok == 0 && (ipBan || macBan) && !c.isGm()) {
            ...
            if (macBan) {
                ...
                MapleCharacter.ban(c.getSession().getRemoteAddress().toString().split(":")[0], "Thûc thi l½nh c¬m tài kho¡n, tài kho¡n" + login, false, 4, false);
            }
        }
        if (loginok != 0) {
            if (!loginFailCount(c)) {
    ...
    public static void CharlistRequest(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (chars != null && ChannelServer.getInstance(channel) != null) {
            ...
            c.getSession().write(LoginPacket.getCharList(c.getPic(), chars, c.getCharacterSlots()));
        } else {
            c.getSession().close();
        }
    }
    ...
    public static void CreateChar(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (job == JobType.Hayato) {
            ...
            newchar.setLevel((short) 20);
            newchar.getStat().str = 107;
            newchar.getStat().dex = 4;
            newchar.getStat().maxhp = 500;
            newchar.getStat().hp = 500;
            newchar.getStat().maxmp = 1000;
            newchar.getStat().mp = 1000;
            newchar.setRemainingSp(33);
        }
        if (job == JobType.Phantom) {
            newchar.setJob((short) 2400);
            newchar.setLevel((short) 10);
            newchar.getStat().dex = 4;
            newchar.getStat().luk = 57;
            newchar.getStat().maxhp = 350;
            newchar.getStat().hp = 350;
            newchar.getStat().maxmp = 350;
            newchar.getStat().mp = 350;
            newchar.setRemainingSp(3);
        }
        if (job == JobType.Demon) {
            newchar.setLevel((short) 10);
            newchar.getStat().dex = 4;
            newchar.getStat().str = 57;
        }
    ...
    public static void DeleteChar(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (c.getPic()!= null) { // On the server, there's a second password
            if (Secondpw_Client == null) { // Client's hacking
                ...
                if (!c.checkPic(Secondpw_Client)) { // Wrong Password
                    state = 20;
                }
            }
        }
    ...
    public static void Character_WithSecondPassword(final LittleEndianAccessor slea, final MapleClient c, final boolean view) {
        ...
        if (!c.isLoggedIn() || loginFailCount(c) || c.getPic()== null || !c.login_Auth(charId) || ChannelServer.getInstance(c.getChannel()) == null || !WorldOption.isExists(c.getWorld())) {
            c.getSession().close();
            return;
        }
        c.updateMacs(slea.readMapleAsciiString());
