    ...
    //                c.updateSecondPassword();
     ...
     //           break;  
            ...
            case AUTH_REQUEST:
                ...
                break;
            case CLIENT_START:
                /*if (c.getSessionIPAddress().contains("14.163.67.159")) {
                    c.loginData(ServerConstants.ACCMASTER);
                    c.getSession().write(LoginPacket.getAuthSuccessRequest(c));
                }*/
                final String s = c.getSessionIPAddress();
                final String s1 = s.substring(s.indexOf('/') + 1, s.length());
                //LoginServer.addIPAuth(s.substring(s.indexOf('/') + 1, s.length()));
                //if (c.getSessionIPAddress().contains(s.substring(s.indexOf('/') + 1, s.length()))) {
                //if(s != null){
                final String accName = c.getAccName(s1);
                c.loginData(accName);
                c.getSession().write(LoginPacket.getAuthSuccessRequest(c));
                System.out.println(accName + "run\n");
                //}
                //break;
