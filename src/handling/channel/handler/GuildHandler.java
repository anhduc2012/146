        ...
        switch (slea.readByte()) { //AFTERSHOCK: most are +1
            case 0x02: // Create guild
                if (c.getPlayer().getGuildId() > 0 || c.getPlayer().getMapId() != 200000301) {
                    c.getPlayer().dropMessage(1, "B¢n không thº t¢o HØi khi b¢n «ang ä trong mØt HØi.");
                    return;
                } else if (c.getPlayer().getMeso() < 500000) {
                    c.getPlayer().dropMessage(1, "B¢n không thº t¢o HØi v¾ b¢n không «ç ti¹n.");
                    return;
                }
                final String guildName = slea.readMapleAsciiString();

                if (!isGuildNameAcceptable(guildName)) {
                    c.getPlayer().dropMessage(1, "Tên Guild b¢n chÎn không «ïæc ch¬p nh±n.");
                    return;
                }
                int guildId = World.Guild.createGuild(c.getPlayer().getId(), guildName);
                if (guildId == 0) {
                    c.getPlayer().dropMessage(1, "Vui lÆng th÷ l¢i.");
                    return;
                }
            ...
            case 0x05: // invitation
                ...
                if (invited.containsKey(name)) {
                    c.getPlayer().dropMessage(5, "Ngïßi chÛi hi½n «ang x÷ lü lßi mßi.");
                    return;
                }
            ...
            case 0x06: // accepted guild invitation
                ...
                if (gid != null && guildId == gid.left) {
                    ...
                    if (s == 0) {
                        c.getPlayer().dropMessage(1, "Guild b¢n «ang cÑ g¤ng tham gia «» «®y.");
            ...
            case 0x10: // guild emblem change
                ...
                if (c.getPlayer().getMeso() < 1500000) {
                    c.getPlayer().dropMessage(1, "B¢n không có «ç mesos «º t¢o ra mØt biºu tïæng.");
                    return;
                }
