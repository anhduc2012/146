...
//                c.getPlayer().getMap().broadcastMessage(MainPacketCreator.MegidoFlameRe(c.getPlayer().getId(), unk, slea.readInt()));
        ...
        if (vip == 1) {
            if (addrem == 0) {
                ...
                if (!FieldLimitType.VipRock.check(chr.getMap().getFieldLimit())) {
                    ...
                    chr.dropMessage(1, "B¡n «Ó này không có s§n «º nh±p vào danh sách.");
                }
            }
        } else if (vip == 2) {
            if (addrem == 0) {
    ...
    public static void TakeDamage(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if ((type != -2) && (type != -3) && (type != -4)) {
            ...
            if (chr.getMapId() == 915000300) {
                ...
                chr.dropMessage(5, "B¢n «» «ïæc phát hi½n ra! Retreat!");
    ...
    public static void CancelBuffHandler(int sourceid, MapleCharacter chr) {
        ...
        Skill skill = SkillFactory.getSkill(sourceid);
            ...
            case 1311013: { //비홀더 도미넌트
                chr.send(JobPacket.CancelHeholderBuff());
                break;   
            }
           case 4341052: 
    ...
    public static void SkillEffect(LittleEndianAccessor slea, MapleCharacter chr) {
        ...
        SkillEffectEntry entry = new SkillEffectEntry(slea.readInt(), slea.readByte(), slea.readByte(), slea.readByte(), slea.readByte());
        ...
        if (skilllevel_serv > 0 && skilllevel_serv == entry.getLevel() && skill.isChargeSkill() && entry.getLevel() > 0 || skill.getId() == 35101009) {
            ...
            if (skill.getId() == 27101202 || skill.getId() == 35001001 || skill.getId() == 35101009 || skill.getId() == 2221011) {
                ...
                //chr.getMap().broadcastMessage(chr, CField.skillEffect2(chr, entry, chr.getPosition()), false);
                chr.getMap().broadcastMessage(chr, CField.skillEffect(chr, skillId, level, direction, unk), false);
            } else {
                chr.getMap().broadcastMessage(chr, CField.skillEffect2(chr, entry, chr.getPosition()), false);
            }
...
//        System.err.println(skillLevel);
            ...
            for (MapleEventType t : MapleEventType.values()) {
                ...
                if ((e.isRunning()) && (!chr.isGM())) {
                    for (int i : e.getType().mapids) {
                        if (chr.getMapId() == i) {
                            chr.dropMessage(5, "B¢n không thº s÷ dîng nó ä «ây.");
                            return;
                        }
                    }
                }
        ...
        switch (skillid) {
           ...
           case 5201008: { //infinite blast Handler
            ...
            case 1311013: { //비홀더 도미넌트
                c.getSession().write(JobPacket.giveBeholderDominant(1301013, 1311013, 0));
                c.getSession().write(CWvsContext.enableActions());
                break;
            }
            case 1311014: { //비홀더 부가 스킬.
                if (skillid == 1311013) {
                    c.getSession().write(JobPacket.giveBeholderDominant(1301013, 1311013, 1311014));
                } else {
                    c.getSession().write(JobPacket.giveBeholderDominant(1301013, 1301013, 1311014));
                }
                c.getSession().write(CWvsContext.enableActions());
                break;
            }
            case 30001061:
                ...
                if (mob != null) {
                    ...
                    if (success) {
                        ...
                        chr.dropMessage(5, "Con quái v±t có quá nhi¹u sñc m¢nh thº ch¬t, nên b¢n không thº b¤t «ïæc nó.");
                    }
                }
                c.getSession().write(CWvsContext.enableActions());
                break;
            case 30001062:
                chr.dropMessage(5, "Không có quái v±t nào có thº «ïæc tri½u t±p. N¤m b¤t quái v±t trïÜc.");
                c.getSession().write(CWvsContext.enableActions());
                break; 
            case 31221001:
            //case 36100010:
            ...
            case 33101005:
                ...
                if (mob != null) {
                    ...
                    chr.dropMessage(5, "Không con quái v±t nào bÅ hút. K¸ n£ng th¬t b¢i.");
                }
                c.getSession().write(CWvsContext.enableActions());
                break;
            case 20040216:
          ...
          //  case 20041239:
            ...
            case 2221011: { //프리징 브레스
                final byte freezemobs = slea.readByte();
                for (int i = 0; i < freezemobs; i++) {
                    int mobId = slea.readInt();
                    MapleMonster mob1 = chr.getMap().getMonsterByOid(mobId);
                }
...
//                    c.getSession().write(AngelicPacket.showRechargeEffect());
        ...
        int projectile = 0;
        int visProjectile = 0;
        if ((!AOE) && chr.getBuffedValue(MapleBuffStat.SOULARROW) == null && attack.skill != 4111004
            ...
            if (chr.getBuffedValue(MapleBuffStat.SPIRIT_CLAW) == null && !termed) {
    ...
    public static void MoveAndroid(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {//MIXTAMAL6
        ...
        final List<LifeMovementFragment> res = MovementParse.parseMovement(slea);//MIXTAMAL6
    ...
    public static void MoveHaku(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        slea.skip(17);
        List res = MovementParse.parseMovement(slea);
    ...
    public static void MovePlayer(LittleEndianAccessor rh, MapleClient c, MapleCharacter chr) {
        ...
        if (res != null) { // TODO more validation of input data
            ...
            if (chr.isHidden()) {
                ...
                map.broadcastMessage(chr, CField.movePlayer(chr.getId(), res, Original_Pos), false);
            }
