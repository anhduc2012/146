...
public class ItemMakerHandler {
    ...
    /*public static final void StartHarvest(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if ((reactor != null) && (reactor.isAlive()) && (reactor.getReactorId() <= 200011) && (chr.getStat().harvestingTool > 0) && (reactor.getTruePosition().distanceSq(chr.getTruePosition()) <= 10000.0D)) {
            if (c.getPlayer().getFatigue() < (GameConstants.GMS ? 200 : 100));
        } else {
            return;
        }
        ...
        if (lastTime + 5000L > System.currentTimeMillis()) {
            ...
            c.getSession().write(CField.harvestMessage(reactor.getObjectId(), GameConstants.GMS ? 13 : 11));
    ...
    public static final void StartHarvest(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (reactor == null || !reactor.isAlive() || reactor.getReactorId() > 200011 || chr.getStat().harvestingTool <= 0 || reactor.getTruePosition().distanceSq(chr.getTruePosition()) > 10000 || c.getPlayer().getFatigue() >= (GameConstants.GMS ? 200 : 100) || !c.getPlayer().isAlive()) {
            return;
        }
        Item item = c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem((short) c.getPlayer().getStat().harvestingTool);
        if (item == null || ((Equip) item).getDurability() == 0) {
    ...
    public static final void CraftEffect(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if ((profession != null) && ((c.getPlayer().getProfessionLevel(profession.intValue()) > 0) || ((profession.intValue() == 92040000) && (chr.getMap().getExtractorSize() > 0)))) {
            ...
            c.getSession().write(CField.EffectPacket.showOwnCraftingEffect(effect, time, effect.endsWith("Extract") ? 1 : 0));
            chr.getMap().broadcastMessage(chr, CField.EffectPacket.showCraftingEffect(chr.getId(), effect, time, effect.endsWith("Extract") ? 1 : 0), false);
        }
    }
