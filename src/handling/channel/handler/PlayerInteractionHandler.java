...
//System.err.println(action);
        switch (action) { // Mode
                    ...
                    if (ob instanceof IMaplePlayerShop && chr.getPlayerShop() == null) {
                        ...
                        if (ob instanceof HiredMerchant) {
                            ...
                            if (!merchant.isOpen() || !merchant.isAvailable()) {
                                chr.dropMessage(1, "C÷a hàng này «ang «ïæc b¡o tr¾, vui lÆng «ªn sau.");
                            } else {
                                if (ips.getFreeSlot() == -1) {
                                    chr.dropMessage(1, "C÷a hàng này «» «¢t công su¬t tÑi «a, vui lÆng «ªn sau.");
                                } else if (merchant.isInBlackList(chr.getName())) {
                                    chr.dropMessage(1, "B¢n «» bÅ c¬m tö c÷a hàng này.");
                                } else {
                                    chr.setPlayerShop(ips);
                                ...
                                if (ips.getFreeSlot() < 0 || ips.getVisitorSlot(chr) > -1 || !ips.isOpen() || !ips.isAvailable()) {
                                    ...
                                    if (slea.available() > 0 && slea.readByte() > 0) { //a password has been entered
                                        ...
                                        if (!pass.equals(ips.getPassword())) {
                                            c.getPlayer().dropMessage(1, "M±t kh¯u chính xác.");
                                            return;
                                        }
                                    } else if (ips.getPassword().length() > 0) {
                                        c.getPlayer().dropMessage(1, "M±t kh¯u chÄnh xác.");
                                        return;
                                    }
                                    chr.setPlayerShop(ips);
                                    ips.addVisitor(chr);
            ...
            case HIRED_MERCHANT_MAINTENANCE: {
                ...
                if (!c.checkPic(password) || password.length() < 6 || password.length() > 16) {
            ...
            case OPEN1: {
                ...
                if (shop != null && shop.isOwner(chr) && shop.getShopType() < 3 && !shop.isAvailable()) {
                    if (chr.getMap().allowPersonalShop()) {
                        if (c.getChannelServer().isShutdown()) {
                            chr.dropMessage(1, "Máy chç chu¯n bÅ t¤t.");
                            c.getSession().write(CWvsContext.enableActions());
...
//            case SET_ITEMS: {
		...
		FilePrinter.printError(FilePrinter.ITEM_TRADE + chr.getName() + ".txt",chr.getName() + " Trade item: "+ item.getItemId() +"("+itemName+") so luong " + item.getQuantity());  
