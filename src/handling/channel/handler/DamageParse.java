...
public class DamageParse {
    ...
    public static void applyAttack(AttackInfo attack, Skill theSkill, MapleCharacter player, int attackCount, double maxDamagePerMonster, MapleStatEffect effect, AttackType attack_type) {
        ...
        if ((attack.real) && (GameConstants.getAttackDelay(attack.skill, theSkill) >= 100)) {
            if(ServerConstants.CHECK_FAST_ATTACK)
        ...
        if ((attack.hits > 0) && (attack.targets > 0)) {
            if (!player.getStat().checkEquipDurabilitys(player, -1)) {
                player.dropMessage(5, "MØt m¶t hàng «» hªt «Ø b¹n nhïng không có phÆng kiºm kê «º «i «ªn.");
                return;
            }
        }
        int totDamage = 0;
            ...
            if (ServerConstants.AntiKS) {
                ...
                if (monster.getBelongsToSomeone()) {
                    if (monster.getBelongsTo() != player.getId() && !player.isGM()) {
                        player.dropMessage(5, "B¢n không thº «ánh quái v±t này v¾ nó thuØc v¹ ngïßi khác.");
                        continue;
                    }
                    if (player.isGM()) {
                        player.dropMessage(5, "B¢n «ang t¬n công mØt con quái v±t «» «ïæc mØt ngïßi chÛi khác «ánh d¬u.");
                    }
                }
            }
                    ...
                    if (player.getJob() >= 310 && player.getJob() <= 312) {
                        if (player.getSkillLevel(3101009) > 0) {
                            Skill skill = SkillFactory.getSkill(3101009);
                                if (attack.skill > 0) {
                                    if (player.QuiverCallCount == 2 && player.getQuiverArrowCount(1) <= 0 && player.getQuiverArrowCount(1) > -10) {
                                        player.setQuiverArrowCount(1, player.getQuiverArrowCount(1) - 1);
                                        player.send(CField.QuiverKartrige(player, 1, player.getQuiverArrowCount(1), true));
                                        if (player.getBuffedValue(MapleBuffStat.ADVANCED_QUIVER) != null) {
                                            if (Randomizer.nextInt(100) < 70) {
                                                int recover = (totDamageToOneMonster * (30 / 100));
                                                player.addHP(recover);
                                            }
                                        } else {
                                            if (Randomizer.nextInt(100) < 50) {
                                                int recover = (totDamageToOneMonster * (20 / 100));
                                                player.addHP(recover);
                                            }
                                        }
                                    } else if (player.QuiverCallCount == 3 && player.getQuiverArrowCount(2) <= 0 && player.getQuiverArrowCount(2) > -10) {                                          
                                        player.setQuiverArrowCount(2, player.getQuiverArrowCount(2) - 1);
                                        player.send(CField.QuiverKartrige(player, 2, player.getQuiverArrowCount(2), true));
                                        if (Randomizer.nextInt(100) < 90) {
                                            MapleStatEffect eff = skill.getEffect(player.getTotalSkillLevel(skill));
                                            MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(MonsterStatus.POISON, Integer.valueOf(1), skill.getId(), null, false);
                                            monster.applyStatus(player, monsterStatusEffect, false, eff.getY() * 5000, true, eff);
                                        }
                                    } else if (player.QuiverCallCount == 4 && player.getQuiverArrowCount(3) <= 0 && player.getQuiverArrowCount(3) > -10) {
                                        player.setQuiverArrowCount(3, player.getQuiverArrowCount(3) - 1);
                                        player.send(CField.QuiverKartrige(player, 3, player.getQuiverArrowCount(3), true));
                                    if (player.getBuffedValue(MapleBuffStat.ADVANCED_QUIVER) != null) {
                                        if (Randomizer.nextInt(100) < 60) {
                                            player.send(CField.giveMagicArrow(player));
                                        }
                                    } else {
        ...
        if ((attack.skill != 0) && ((attack.targets > 0) || ((attack.skill != 4331003) && (attack.skill != 4341002))) 
            && attack.skill != 31121005 && (!GameConstants.isNoDelaySkill(attack.skill))) {
    ...
    public static AttackInfo parseDmgM(LittleEndianAccessor lea, MapleCharacter chr)//reg att
    {
       ...
       // lea.readInt(); //same as above
        ...
        switch (ret.skill) {
            ...
            case 2211007:
            case 12111007: //Teleport Mastery cygnus mage
            case 22161005: // Teleport Mastery evan
            case 32111010:
