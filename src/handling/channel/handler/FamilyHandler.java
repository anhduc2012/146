...
public class FamilyHandler {
        ...
        switch (entry) {
            case Teleport: //teleport: need add check for if not a safe place
                ...
                if (FieldLimitType.VipRock.check(c.getPlayer().getMap().getFieldLimit()) || c.getPlayer().isInBlockedMap()) {
                    c.getPlayer().dropMessage(5, "Tri½u hÓi th¬t b¢i. VÅ trí ho¶c tiºu bang hi½n t¢i cça b¢n không cho phép gi¬y tri½u t±p.");
                    success = false;
                } else if (victim == null || (victim.isGM() && !c.getPlayer().isGM())) {
                    c.getPlayer().dropMessage(1, "Tên không hæp l½ ho¶c b¢n không ä trên cùng mØt kênh.");
                    success = false;
            ...
            case Summon: // TODO give a check to the player being forced somewhere else..
                ...
                if (FieldLimitType.VipRock.check(c.getPlayer().getMap().getFieldLimit()) || c.getPlayer().isInBlockedMap()) {
                    ...
                    c.getPlayer().dropMessage(1, "MØt nhân v±t khác «» yêu c®u tri½u hÓi nhân v±t này. Vui lÆng th÷ l¢i sau.");
                } else if (victim.getFamilyId() == c.getPlayer().getFamilyId() && !FieldLimitType.VipRock.check(victim.getMap().getFieldLimit()) && victim.getId() != c.getPlayer().getId() && !victim.isInBlockedMap()) {
    ...
    public static final void FamilyOperation(final LittleEndianAccessor slea, MapleClient c) {
        ...
        if (addChr == null) {
            c.getPlayer().dropMessage(1, "Tên b¢n yêu c®u không «úng ho¶c anh / cô ¬y hi½n không «£ng nh±p.");
        } else if (addChr.getFamilyId() == c.getPlayer().getFamilyId() && addChr.getFamilyId() > 0) {
            c.getPlayer().dropMessage(1, "B¢n thuØc cùng mØt gia «¾nh.");
        } else if (addChr.getMapId() != c.getPlayer().getMapId()) {
            c.getPlayer().dropMessage(1, "MØt trong nhøng b¢n muÑn thêm nhï là mØt cÛ sä ph¡i trên cùng mØt b¡n «Ó.");
        } else if (addChr.getSeniorId() != 0) {
            c.getPlayer().dropMessage(1, "Nhân v±t «» là mØt cÛ sä cça mØt nhân v±t khác.");
        } else if (addChr.getLevel() >= c.getPlayer().getLevel()) {
            c.getPlayer().dropMessage(1, "CÛ sä b¢n muÑn thêm ph¡i ä c¬p th¬p hÛn.");
        } else if (addChr.getLevel() < c.getPlayer().getLevel() - 20) {
            c.getPlayer().dropMessage(1, "Kho¡ng cách giøa b¢n và cÛ sä cça b¢n ph¡i n¥m trong 20 c¬p «Ø.");
        } else if (addChr.getLevel() < 10) {
            c.getPlayer().dropMessage(1, "CÛ sä b¢n muÑn thêm ph¡i vïæt quá C¬p 10.");
        } else if (c.getPlayer().getJunior1() > 0 && c.getPlayer().getJunior2() > 0) {
            c.getPlayer().dropMessage(1, "B¢n «» có 2 c¬p dïÜi.");
    ...
    public static final void FamilySummon(final LittleEndianAccessor slea, MapleClient c) {
        ...
        if (c.getPlayer().getFamilyId() > 0 && tt != null && tt.getFamilyId() == c.getPlayer().getFamilyId() && !FieldLimitType.VipRock.check(tt.getMap().getFieldLimit())
            ...
            if (accepted) {
                ...
                tt.dropMessage(5, "Tri½u hÓi th¬t b¢i. VÅ trí ho¶c tiºu bang hi½n t¢i cça b¢n không cho phép gi¬y tri½u t±p.");
            }
        } else {
            c.getPlayer().dropMessage(5, "Tri½u hÓi th¬t b¢i. VÅ trí ho¶c tiºu bang hi½n t¢i cça b¢n không cho phép gi¬y tri½u t±p.");
        }
    ...
    public static final void DeleteSenior(final LittleEndianAccessor slea, MapleClient c) {
        ...
        //}
        c.getPlayer().saveFamilyStatus();
        MapleCharacterUtil.sendNote(mgc.getName(), c.getPlayer().getName(), c.getPlayer().getName() + " «» yêu c®u c¤t ngang vÜi b¢n, v¾ v±y mÑi quan h½ gia «¾nh «» kªt thúc.", 0);
