...
public class ChannelServer {
    ...
    public boolean manualEvent(MapleCharacter chr) {
        ...
        if (manualEvent) {
            chr.dropMessage(5, "Sû ki½n thç công có " + (manualEvent ? "began" : "begone") + ".");
        }
        return manualEvent;
    }
    ...
    public void warpToEvent(MapleCharacter chr) {
        if (!manualEvent || manualEventMap <= 0) {
            chr.dropMessage(5, "Không có sû ki½n nào di¼n ra.");
            return;
        }
        chr.dropMessage(5, "B¢n «ang di chuyºn vào b¡n «Ó sû ki½n.");
