        ...
        switch (type) {
            case item: {
                ...
                for (QuestItem item : items) {
                    ...
                    if (count < 0) { // remove items
                        if (!c.haveItem(id, count, false, true)) {
                            c.dropMessage(1, "B¢n thiªu mØt sÑ v±t ph¯m «º hoàn thành nhi½m vî.");
                            return false;
                        }
                    } else { // add items
                        if (MapleItemInformationProvider.getInstance().isPickupRestricted(id) && c.haveItem(id, 1, true, false)) {
                            c.dropMessage(1, "B¢n «» có m¶t hàng này: " + MapleItemInformationProvider.getInstance().getName(id));
                            return false;
                        }
                ...
                if (c.getInventory(MapleInventoryType.EQUIP).getNumFreeSlot() < eq) {
                    c.dropMessage(1, "Vui lÆng làm trÑng mîc Equip cça túi hành lü.");
                    return false;
                } else if (c.getInventory(MapleInventoryType.USE).getNumFreeSlot() < use) {
                    c.dropMessage(1, "Vui lÆng làm trÑng mîc Use cça túi hành lü.");
                    return false;
                } else if (c.getInventory(MapleInventoryType.SETUP).getNumFreeSlot() < setup) {
                    c.dropMessage(1, "Vui lÆng làm trÑng mîc Setup cça túi hành lü.");
                    return false;
                } else if (c.getInventory(MapleInventoryType.ETC).getNumFreeSlot() < etc) {
                    c.dropMessage(1, "Vui lÆng làm trÑng mîc Etc cça túi hành lü.");
                    return false;
                } else if (c.getInventory(MapleInventoryType.CASH).getNumFreeSlot() < cash) {
                    c.dropMessage(1, "Vui lÆng làm trÑng mîc Cash cça túi hành lü.");
                    return false;
                }
                return true;
            }
            case money: {
                ...
                if (c.getMeso() + meso < 0) { // Giving, overflow
                    c.dropMessage(1, "ThÅt vïæt quá sÑ ti¹n tÑi «a, 9999999999.");
                    return false;
