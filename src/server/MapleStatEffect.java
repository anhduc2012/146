...
public class MapleStatEffect implements Serializable {
    ...
    public static final MapleStatEffect loadSkillEffectFromData(final MapleData source, final int skillid, final boolean overtime, final int level) {
        return loadFromData(source, skillid, true, overtime, level , null);
    }
     ...
     //               ret.info.put(MapleStatInfo.targetPlus, 2);
      ...
      //              break;
                ...
                case 24120002:
                    ...
                    break;
                case 11121203:
                    ret.info.put(MapleStatInfo.attackCount, 7);
                    break;  
            }
        ...
        if (ret.skill) { // hack because we can't get from the datafile...
            ...
            if (!found) {
                ...
                case 2221011: //프리징 브레스
                    //ret.statups.add(new Triple(MapleBuffStat.FRIZING_BRESS, 1, false));
...
//                        break;
                ...
                //case 3211012: // 익스트림 아쳐리 : 석궁
                            //ret.statups.put(MapleBuffStat.EXTREAM_ARCH, ret.info.get(MapleStatInfo.x));
                    //break;
                        case 15001022:
                        ...
                        case 41001001: // Battoujutsu Stance
                            ret.info.put(MapleStatInfo.time, Integer.valueOf(Integer.MAX_VALUE));
                ...
                case 31111005:
                    ...
                    break;
                        ...
                        case 1311014: // Evil Eye of Domination
                            //ret.statups.put(MapleBuffStat.BEHOLDER, Integer.valueOf(ret.level));
                            break;
                        case 1311013:
                            //ret.statups.put(MapleBuffStat.BEHOLDER, ret.info.get(MapleStatInfo.x));
                            break;
                        ...
                        case 31121005:
                            ...
                            ret.statups.put(MapleBuffStat.HP_R, (int) ret.info.get(MapleStatInfo.mhpR));
                            break;
                        case 2311003: // hs
                        case 9001002: // GM hs
                        case 9101002:
...
//                            ret.statups.put(MapleBuffStat.HOLY_SHIELD, GameConstants.GMS ? (int) ret.level : ret.info.get(MapleStatInfo.x));
                        ...
                        case 32110007:
                            ret.info.put(MapleStatInfo.time, (sourceid == 32110007 ? 60000 : Integer.MAX_VALUE));
                        ...
                        case 32110000:
                        case 32110008:
                            ret.info.put(MapleStatInfo.time, (sourceid == 32110008 ? 60000 : Integer.MAX_VALUE));
                            ret.statups.put(MapleBuffStat.AURA, (int) ret.level);
                        ...
                        case 32110009:
                            ret.info.put(MapleStatInfo.time, (sourceid == 32110009 ? 60000 : Integer.MAX_VALUE));
                            ret.statups.put(MapleBuffStat.AURA, (int) ret.level);
    ...
    public final boolean applyTo(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final Point pos, int newDuration) {
        if (isHeal() && (applyfrom.getMapId() == 749040100 || applyto.getMapId() == 749040100)) {
            ...
            applyfrom.dropMessage(5, "Bạn không thể sử dụng kỹ năng này ở các thị trấn.");
            applyfrom.getClient().getSession().write(CWvsContext.enableActions());
            return false;
        }
        ...
        if (summonMovementType != null && (sourceid != 32111006 || (applyfrom.getBuffedValue(MapleBuffStat.REAPER) != null && !primary)) && !applyto.isClone()) {
            int summId = sourceid;
            if (applyfrom.isGM()) {
                applyfrom.dropMessage(5,"SkillID : " + sourceid + " Pos : X : " + pos.x + " Y : " + pos.y);
            }
            if (sourceid == 3111002) {
        ...
        if (isMagicDoor()) { // Magic Door
            ...
            if (door.getTownPortal() != null) {
                ...
                applyto.dropMessage(5, "Bạn không thể sinh ra một cánh cửa bởi vì tất cả cửa ra vào trong thành phố được lấy.");
            }
        } else if (isMist()) {
    ...
    public final boolean applyReturnScroll(final MapleCharacter applyto) {
        if (moveTo != -1) {
            if (applyto.getMap().getReturnMapId() != applyto.getMapId() || sourceid == 2031010 || sourceid == 2030021 || sourceid == 20021110 || sourceid == 2030028 || sourceid == 20031203) {
                ...
                } else if (sourceid == 2031203 && moveTo == 150000000) {
                    target = ChannelServer.getInstance(applyto.getClient().getChannel()).getMapFactory().getMap(moveTo);
                } else if (sourceid == 2031010 && moveTo == 310010000) {
                    target = ChannelServer.getInstance(applyto.getClient().getChannel()).getMapFactory().getMap(moveTo);
            ...
            }  case 15001022://LIGHTNING Elemental ThunderBreaker
{
            ...
            case 3101009: { // 퀴버 카트리지
                if (applyto.QuiverCallCount == 0) {
                    ...
                    applyto.getClient().getSession().write(CField.QuiverKartrige(applyto, 1, info.get(MapleStatInfo.y), false));
                    applyto.QuiverCallCount++;
                } else if (applyto.QuiverCallCount == 2) {
                    applyto.setQuiverArrowCount(2, 0);
                    applyto.getClient().getSession().write(CField.QuiverKartrige(applyto, 2, info.get(MapleStatInfo.y), false));
                    applyto.QuiverCallCount++;
                } else if (applyto.QuiverCallCount == 3) {
                    applyto.setQuiverArrowCount(3, 0);
                    applyto.getClient().getSession().write(CField.QuiverKartrige(applyto, 3, info.get(MapleStatInfo.y), false));
                    applyto.QuiverCallCount++;
                } else {
    ...
    public final boolean isBeholder() {
        return skill && sourceid == 1311014 || sourceid == 1301013 || sourceid == 1311013;
    }
