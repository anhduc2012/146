...
public abstract class MapleEvent {
    ...
    public static void givePrize(final MapleCharacter chr) {
        ...
        if (reward == 0) {
            ...
            chr.dropMessage(5, "B¢n nh±n «ïæc " + mes + " Mesos.");
        } else if (reward == 1) {
            final int cs = Randomizer.nextInt(4000) + 1000;
            chr.modifyCSPoints(1, cs, true);
            chr.dropMessage(5, "B¢n nh±n «ïæc " + (GameConstants.GMS ? (cs / 2) : cs) + " cash.");
        } else if (reward == 2) {
            chr.setVPoints(chr.getVPoints() + 1);
            chr.dropMessage(5, "B¢n nh±n «ïæc 1 Vote Point.");
        } else if (reward == 3) {
            chr.addFame(10);
            chr.dropMessage(5, "B¢n nh±n «ïæc 10 Fame.");
	} else if (reward == 4) {
	    chr.dropMessage(5, "Không có ph®n thïäng.");
        } else {
            int max_quantity = 1;
            switch (reward) {
                case 5062000:
