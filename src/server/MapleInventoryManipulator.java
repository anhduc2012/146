...
public class MapleInventoryManipulator {
            ...
            if (type == MapleInventoryType.ETC && dst > 100 && dst % 100 != 0) {
                ...
                if (eSlot > 0) {
                    ...
                    if (dst % 100 > ee.getSlotCount() || ee.getType() != ii.getBagType(source.getItemId()) || ee.getType() <= 0) {
                        c.getPlayer().dropMessage(1, "B¢n không thº di chuyºn v±t ph¯m vào túi.");
                        c.getSession().write(CWvsContext.enableActions());
                        return;
                    } else {
                        eqIndicator = 0;
                ...
                c.getPlayer().dropMessage(1, "B¢n không thº di chuyºn nó ä «ó.");
                c.getSession().write(CWvsContext.enableActions());
                return;
            }
        }
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (GameConstants.isGMEquip(source.getItemId()) && !c.getPlayer().isStaff()) {
            c.getPlayer().dropMessage(1, "Ch¿ Game Master mÜi «ïæc phép s÷ dîng mîc này.");
        ...
        if (GameConstants.isOverPoweredEquip(c, source.getItemId(), src) && !c.getPlayer().isStaff()) {
            c.getPlayer().dropMessage(1, "Dïßng nhï mîc này quá c¬p nguÓn, vui lÆng báo cáo cho Qu¡n trÅ viên nªu b¢n cho r¥ng h½ thÑng không «úng.");
        ...
        if (!c.getPlayer().isGM()) {
            if (source.getItemId() == 1112663 || source.getItemId() == 1112586) {
                c.getPlayer().dropMessage(1, "White Angelic Blessing và Dark Angelic Blessing hi½n không ho¢t «Øng.");
                c.getSession().write(CWvsContext.enableActions());
                return;
            }
        }
                ...
                if (s.id.contains(Integer.valueOf(source.getItemId()))) {
                    ...
                    for (Integer i : s.id) {
                        if (theList.contains(i)) {
                            c.getPlayer().dropMessage(1, "B¢n không thº trang bÅ m¶t hàng này v¾ b¢n «» có " + (StringUtil.makeEnumHumanReadable(s.name())) + " equipped.");
                            c.getSession().write(CWvsContext.enableActions());
                            return;
                        }
                    }
    ...
    public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, short quantity, final boolean npcInduced) {
        ...
        final Item source = c.getPlayer().getInventory(type).getItem(src);
        String itemName = MapleItemInformationProvider.getInstance().getName(source.getItemId());
        ...
        if (quantity < source.getQuantity() && !GameConstants.isRechargable(source.getItemId())) {
           ...
           FilePrinter.print(FilePrinter.DROP_ITEM + c.getPlayer().getName() + ".txt", c.getPlayer().getName() + " Drop: "+source.getItemId()+"("+itemName+") so luong: "+source.getQuantity()+" in map: "+c.getPlayer().getMapId()+"\r\n");  
