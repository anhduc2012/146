      ...
      public void showMapleLeafScene() {
        try {
         ...
         c.getPlayer().HonorUnlock();
          c.getPlayer().dropMessage(5, "Khe 1 Mä ra ti¹m n£ng bên trong.");
    }
        
        public final void UnlockHonor2() {
         c.getPlayer().HonorUnlock2();
         c.getPlayer().dropMessage(5, "Khe 2 Mä ra ti¹m n£ng bên trong.");
    }
                
         public final void UnlockHonor3() {
         c.getPlayer().HonorUnlock3();
         c.getPlayer().dropMessage(5, "Khe 3 Mä ra ti¹m n£ng bên trong.");
    }
...
//            return true;
    ...
    public String checkDrop(int mobId) {
        final List<MonsterDropEntry> ranks = MapleMonsterInformationProvider.getInstance().retrieveDrop(mobId);
        if (ranks != null && ranks.size() > 0) {
            int num = 0, itemId = 0, ch = 0,chance =0;
            MonsterDropEntry de;
            StringBuilder name = new StringBuilder();
            for (int i = 0; i < ranks.size(); i++) {
                de = ranks.get(i);
                if (de.chance > 0 && (de.questid <= 0 || (de.questid > 0 && MapleQuest.getInstance(de.questid).getName().length() > 0))) {
                    itemId = de.itemId;
                    if (num == 0) {
                        name.append("V±t ph¯m rÛi ra tö  #o" + mobId + "# \r\n- ID : "+ mobId+" | TÏ l½ /10000\r\n");
                        name.append("--------------------------------------\r\n");
                    }
                    String namez = "#z" + itemId + "#";
                    if (itemId == 0) { //meso
                        itemId = 4031041; //display sack of cash
                        namez = (de.Minimum * 1) + " to " + (de.Maximum * 1) + " meso";
                    }
                    //float chance = (float) (1000000 / de.chance / getClient().getChannelServer().getDropRate());
                    ch = (de.chance * 1);
                    chance = (ch / 100);
                    name.append((num + 1) + ") #v" + itemId + "#" + namez + " - " 
                            + (de.questid > 0 && MapleQuest.getInstance(de.questid).getName().length() > 0 ? ("Nhi½m vî : " + MapleQuest.getInstance(de.questid).getName() + ".") : "") 
                            + chance +""
                            + "\r\n"
                    );
                    num++;
        ...
        return "Không có v±t ph¯m nào rÛi tö quái v±t này.";
    }     
