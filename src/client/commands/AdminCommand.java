...
public class AdminCommand {
    ...
    public static class setall extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            if (splitted.length < 1) {
                c.getPlayer().dropMessage(6, "!setall <ch¿ sÑ>");
                return 0;
            }
            final int x = Short.parseShort(splitted[1]);
            c.getPlayer().getStat().setStr((short) x, c.getPlayer());
            c.getPlayer().getStat().setDex((short) x, c.getPlayer());
            c.getPlayer().getStat().setInt((short) x, c.getPlayer());
            c.getPlayer().getStat().setLuk((short) x, c.getPlayer());
            c.getPlayer().updateSingleStat(MapleStat.STR, x);
            c.getPlayer().updateSingleStat(MapleStat.DEX, x);
            c.getPlayer().updateSingleStat(MapleStat.INT, x);
            c.getPlayer().updateSingleStat(MapleStat.LUK, x);
            return 1;
        }
    }
