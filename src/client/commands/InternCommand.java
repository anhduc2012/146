...
public class InternCommand {
    ...
    public static class accname extends CommandExecute {
        ...
        //LoginServer.addIPAuth(s.substring(s.indexOf('/') + 1, s.length()));
                //if (c.getSessionIPAddress().contains(s.substring(s.indexOf('/') + 1, s.length()))) {
                    String accName = c.getAccName(s);
                    c.loginData(accName);
                    c.getPlayer().dropMessage(5, accName + s);
            return 0;
        }
    }
    ...
    public static class WhereAmI extends CommandExecute {
		...
		c.getPlayer().dropMessage(5,"NPCs on this map:");
		for(MapleMapObject npcs : c.getPlayer().getMap().getAllNPCs()){
		...
		c.getPlayer().dropMessage(5,"Portals on this map:");
		...
		c.getPlayer().dropMessage(5,"Reactors on this map:");
		for(MapleReactor reactor : c.getPlayer().getMap().getAllReactor()){
			c.getPlayer().dropMessage(5, ">> " + reactor.getName() + " - " + reactor.getReactorId() + " - " + reactor.getDelay() + " State: " + reactor.getState());
		}
