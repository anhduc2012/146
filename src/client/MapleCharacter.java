...
//            }
            ...
            ps.setInt(1, chr.id);
            ps.setByte(2, (byte) 100); // Eq
            ps.setByte(3, (byte) 100); // Use
            ps.setByte(4, (byte) 100); // Setup
            ps.setByte(5, (byte) 100); // ETC
            ps.setByte(6, (byte) 100); // Cash
            ps.execute();
            ps.close();
...
//            client.getSession().write(EffectPacket.updateDeathCount(99)); //for fun
    ...
    public void cancelChallenge() {
        if (challenge != 0 && client.getChannelServer() != null) {
            ...
            dropMessage(6, "Tö chÑi th÷ thách.");
            challenge = 0;
        }
    }
            ...
            if (!GameConstants.isBeginnerJob(newJob) && !GameConstants.isDualBladeNoSP(newJob) && !GameConstants.isZero(newJob) && !GameConstants.isBeastTamer(newJob) && !GameConstants.isGM(newJob)) {
                ...
                if (!isGM()) {
                    ...
                    if (!GameConstants.isEvan(newJob)) {
                        ...
                        dropMessage(5, "Con rÓng «» nä và dïßng nhï có g¾ «ó «º nói vÜi b¢n. Nh¬p vào con rÓng «º b¤t «®u cuØc trÆ chuy½n.");
                    }
                }
                if (newJob == 433) {
                    remainingSp[GameConstants.getSkillBook(newJob, 0)] -= 2;
...
//        System.out.println(start + " | " + end);
    ...
    public void addMP(int delta, boolean ignore) {
        if ((delta < 0 && GameConstants.isDemonSlayer(getJob())) || !GameConstants.isDemonSlayer(getJob()) || ignore && (this.stats.setMp(this.stats.getMp() + delta, this))) {
...
//            client.getSession().write(InventoryPacket.updateSpecialItemUse(item, type == MapleInventoryType.EQUIPPED ? (byte) 1 : type.getType(), this));
    ...
    public int getTotalSkillLevel(int skillid) { // fix buff summon lien tuc
   ...
   //     client.getSession().write(CField.getAndroidTalkStyle(2008, "Yay", 2));
        ...
        familyUpdate();
        autoJob();
        FilePrinter.print(FilePrinter.LEVEL_UP + name + ".txt", name + " Lv."+level +" in map: "+mapid+".\r\n");	
    ...
    public void autoJob() {
        ...
        } else if (GameConstants.ExPlorer(job)) {
            switch (getLevel()) {
                case 60:
                    changeJob((short) (job + 1));
                    break;
        ...
        } else if (GameConstants.Demon(job)) {
            switch (getLevel()) {
                case 30:
        ...
        } else if (GameConstants.isBatllemage(job)) {
            switch (getLevel()) {
                case 30:
        ...
        }else if (GameConstants.isDawnWarrior(job)) {
            switch (getLevel()) {
                case 30:
        ...
        } else if (GameConstants.isWindArcher(job)) {
            switch (getLevel()) {
                case 30:
                    changeJob((short) 1310);
                    break;
        ...
        } else if (GameConstants.isThunderBreaker(job)) {
            switch (getLevel()) {
                case 30:
                    changeJob((short) 1310);
                    break;
    ...
    public void setSmega() {
        if (smega) {
            smega = false;
            dropMessage(5, "Loa «» t¤t");
        } else {
            smega = true;
            dropMessage(5, "Loa «» b±t");
        }
    }
    ...
    public void unchooseStolenSkill(int skillID) { //base skill
        if (skillisCooling(20031208) || stolenSkills == null) {
            dropMessage(-6, "[Loadout] K¸ n£ng này là dïÜi thßi gian hÓi chiêu. Vui lÆng «æi.");
            return;
        }
...
//    }
    ...
    public void newCharRewards() {
        ...
        rewards.add(1142358);
        for (int reward : rewards) {
            addReward(1, reward, 0, 0, 0, "Nhan qua newbie!");
        }
        updateReward();
    }
...
//        client.getSession().write(EffectPacket.updateDeathCount(deathCount));
    ...
    */
    ...
    public void setQuiverArrowCount(int arrow, int set) {
        if (getBuffedValue(MapleBuffStat.ADVANCED_QUIVER) == null) {
            if (arrow == 1) {
