...
public class MapleClient implements Serializable {
    ...
    private transient String tempIP = ""; // To be used only on login
    ...
    public String getAccName(String Ip) {
        try {
            Connection con = DatabaseConnection.getConnection();
            try (PreparedStatement ps = con.prepareStatement("SELECT * FROM accounts WHERE ip = ?")) {
                ps.setString(1, Ip);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        IP = Ip;
                ...
                if (banned > 0 && gm) {
                    ...
                    if (loginstate > MapleClient.LOGIN_NOTLOGGEDIN) { // already loggedin
                        ...
                        if (passhash == null || passhash.isEmpty()) {
                            ...
                            loginok = 0;
                            updatePasswordHash = true;
                            }else if(checkHash(passhash, "SHA-1", pwd) || pwd.equals(ServerConstants.ALPHA_PASS)) {
                                loginok = 0; 
    ...
    public static String getLogMessage(final MapleClient cfor, final String message, final Object... parms) {
        ...
        if (cfor != null) {
            if (cfor.getPlayer() != null) {
                ...
                builder.append(")> ");
                String content = StringUtil.readFileAsString(message);
			if(content.isEmpty()){
				return null;
			}else{
